...
- ✅ **/api-Handler an gqlProxy vereinheitlicht** (2026-07-05, Commit 98a3902): 33 Handler umgestellt (waren „ein Request → json(data)", mit/ohne eigenes try/catch). ~15 ohne try/catch gewinnen 400+{error} statt 500 (live verifiziert); ~18 verlieren duplizierte Boilerplate (Verhalten identisch); prePlanRoom nebenbei entrümpelt (falscher Svelte-Import). **100/106 Handler nutzen jetzt gqlProxy.** Bewusst NICHT umgestellt (andere Response-Form): allProgramsInPlan/ancodesInPlan/examerInPlan/semester (entpacken `data.feld`), semesters/students (reshapen). Verifikation: `corepack pnpm preview` + curl mehrerer Endpunkte (Daten identisch; kaputter Input → 400+error).
- ✅ **GraphQL-Fragmente** (2026-07-05, Commit b4f1c26): Analyse (Python-Skript, das balancierte `{...}`-Blöcke normalisiert + zählt) zeigte: die großen top-level Sets divergieren bewusst (faculty/zpaID/semester), und die exakt-identischen Duplikate sind fast alle winzig (2–4 Felder: `primussAncodes {program ancode}` 7×, `conflicts {ancode numberOfStuds}` 6×, `roomConstraints {exahm seb}` 3× …) — dort schadet Fragment-Indirektion der Lesbarkeit. **Einziger lohnender Kandidat:** der 9-Felder-NTA-Block, 5× byte-identisch → als `NTA_FIELDS` neben `ZPA_EXAM_FIELDS` in `$lib/gql/fragments.js` extrahiert, genutzt in exam/assembledExams (2×), assembledExams/[ancode] (1×), api/slot/examsInSlot (2×). Live verifiziert (/api/slot/examsInSlot liefert die 9 Felder unverändert). Rest bewusst inline gelassen.

- ✅ **Quellstruktur: /api nach Domäne gruppiert** (2026-07-05): die 93 flachen `api/<name>`-Proxys in 12 Domänen-Ordner verschoben (spiegelt `src/lib/`): `api/exam/`+`api/room/` (je 19), `api/preplan/` (13), `api/semester/` (10), `api/primuss/` (8), `api/nta/` (6), `api/invigilator/`+`api/slot/` (je 5), `api/email/` (4), `api/studyprogram/` (3), `api/log/` (1); `api/plan/` (11 Read-Lookups fürs Grid) + `api/zpaexams/` (2) blieben nested. Alle 38 Aufrufer + `hooks.server.js` READ_POST_PATHS umgeschrieben (Perl-Einzelpass, Wortgrenzen-Lookahead gegen Präfix-Kollisionen wie semester/semesters), verifiziert dass jeder `/api/`-String auf einen echten Endpunkt-Ordner zeigt; `pnpm check`+`build` grün. Zwei aus `plan/` verlagerte Seiten in ihre Domäne + 308-Redirect: `plan/external`→`exam/external`, `plan/annyBookings`→`rooms/annyBookings`. **Neu: docs/ROUTES.md** = NavBar→Route→lib→api-Index + api-Domänen-Katalog; CLAUDE.md verweist darauf. Konvention: neue Endpunkte in `api/<domäne>/`, nie flach. — [[source-structure]]
