...
- ✅ e2e-Smoke-Suite: 39 param-freie Routen (siehe oben).
- **e2e param-Routen** (`[ancode]`/`[code]`/`[mtknr]`/`[day]`): brauchen echte Test26SS-IDs (z. B. aus `examsWithoutSlot`/`ntas` in `beforeAll` ziehen) — Follow-up.
- **17 Sonder-Proxies** an gqlProxy angleichen; **GraphQL-Fragmente** nur bei exakt identischen Feld-Sets.
