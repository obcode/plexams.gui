...
- **Endschalter `compilerOptions.runes: true` — BLOCKIERT durch Event-Syntax:** `sv migrate` (2× lokal gelaufen, Commits 9bcc93e + a04cf64) hat Props/State/`$app/stores`→`$app/state` konvertiert, aber die **Event-Konvertierung crasht reproduzierbar** (svelte migrate bug: `extract_type_and_comment`, MagicString `Cannot set properties of undefined`). Rest für den Endschalter: **~106 `on:`-Direktiven in 31 Dateien** (66× `on:click` überw. nativ; ~40 Component-Events: selected/unselected/done/addToSlot/rmFromSlot/edittime/approve/active/saved/close/…), **10 `createEventDispatcher`**, 3 `export let`, 3 `$:`, 1 `<slot>`, 1 `run()` (InvigilatorDays). MANUELL, koordiniert: je Dispatcher-Komponente Kind→Callback-Prop + alle Eltern `on:X`→`onX`; native `on:click`→`onclick` (aber Component-Forwarding-`on:click` NICHT blind swappen). Rezept in [[svelte-runes-migration]]. Erst wenn 0 `on:`/Dispatcher → `runes:true` einschalten. **App funktioniert einwandfrei** mit Legacy-`on:` im Coexistenz-Modus; das ist reine Guardrail-Arbeit.
