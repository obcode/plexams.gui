...
**OFFEN — kleiner Rest:**
- ✅ `run()`-Blöcke: alle 3 erledigt (siehe oben) — 0 `svelte/legacy` mehr.
- e2e-Smoke-Tests (Playwright/mocked; im Container KEIN Chromium installiert). Als Zwischenlösung möglich: `svelte/server`-`render()`-Tests in node-vitest (kein jsdom) mit `$app/stores`-Mock — belegen SSR-Output ohne Browser (Client-Interaktion/Reassign-Reaktivität deckt das aber NICHT ab). 17 Sonder-Proxies an gqlProxy; GraphQL-Fragmente nur bei identischen Sets.
