...
**OFFEN — kleiner Rest:**
- 3 verbliebene `run()`-Blöcke (ExamConflictsPanel/examsToPlan/SemesterConfigForm) — bewusst belassen (korrekt + SSR-sicher unter runes:true). Umstellung nur mit sorgfältigem Test: tiefe In-place-Mutation der abgeleiteten Liste + Hilfsstrukturen (byAncode) bzw. `bind:` aufs Formular → writable-$derived+Reassign wäre bug-anfällig ohne echtes Browser/SSR-Testing.
