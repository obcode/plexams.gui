...
The plexams backend has a gqlgen middleware that rejects **all** write mutations while *any* validation subscription is running ("writes are blocked while a validation is running"); invigilation generation also refuses to write then. Parallel validations are allowed (counter-guarded).
