<script>
	...
	const roomUsedOnDay = (day, room) =>
		(data.semesterConfig?.starttimes ?? []).some((/** @type {any} */ t) =>
			isPlanned(day, t.number, room)
		);
	...
	async function toggleBlockDay(day, room) {
		...
		const allSlots = (data.semesterConfig?.starttimes ?? []).map(
			(/** @type {any} */ t) => t.number
		);
...
{:else}
	<div class="mx-2 mt-4 flex flex-col gap-4">
		...
		<!-- Toolbar -->
		<div
			class="flex flex-wrap items-center gap-4 rounded-lg border border-base-300 bg-base-100 p-3"
		>
