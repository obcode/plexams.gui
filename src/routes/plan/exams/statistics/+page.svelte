<script>
	...
	let examCountBuckets = $derived(stats?.examCountBuckets ?? []);
...
<div class="mx-auto flex max-w-6xl flex-col gap-6">
		...
		<div>
			...
			{#if stats}
				<p class="mt-1 text-sm text-base-content/70">
					Studierende mit höchstens {stats.maxRegularNonRepeatExams} Nicht-Wiederholungsprüfungen (normaler
					Studienverlauf).
				</p>
	...
	{:else}
		...
		<!-- Ausgeblendete Wiederholer:innen: „kaum abweichend"-Hinweis -->
		{#if stats.excludedStudentCount > 0}
			<p class="-mt-3 text-xs text-base-content/50">
				Ausgeblendet: <span class="tabular-nums">{stats.excludedStudentCount}</span>
				Wiederholer:innen; über alle: {formatShare(stats.allFreeDayShare)} (hier:
				{formatShare(stats.freeDayShare)}) — kaum abweichend.
			</p>
		{/if}
